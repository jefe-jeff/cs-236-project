...
class Info_LFADS_Net(nn.Module):
    ...
    def __init__(self, inputs_dim, T, dt,
        ...
        super(Info_LFADS_Net, self).__init__()
        ...
        # Store loss
        self.full_loss_store = {'train_loss' : {}, 'train_recon_loss' : {}, 'train_kl_loss' : {}, 'train_info_loss' : {},
                                'valid_loss' : {}, 'valid_recon_loss' : {}, 'valid_kl_loss' : {}, 'valid_info_loss' : {},
                                'l2_loss' : {}}
...
#         pdb.set_trace()
        ...
        u = Variable(torch.randn(self.batch_size, self.u_dim).to(self.device))*torch.exp(0.5*self.g0_logvar) \
                        + self.g0_mean
        u_prior = Variable(torch.randn(self.batch_size, self.u_dim).to(self.device))*torch.exp(0.5*self.g0_prior_logvar) \
                        + self.g0_prior_mean

        
        self.info_loss =  (u-u_prior).pow(2).mean()
    ...
    def generate(self, x):
        ...
        for t in range(self.T):
            ...
            u_prior = Variable(torch.randn(self.batch_size, self.u_dim).to(self.device))*torch.exp(0.5*self.u_prior_logvar) \
                        + self.u_prior_mean
            ...
            self.info_loss = self.info_loss + (self.u-u_prior).pow(2).mean()
            # Update generator
    ...
    def test(self, l2_loss, dl=None, dataset=None, batch_size=4):
        ...
        for i, x in enumerate(dl, 0):
            with torch.no_grad():
                x = Variable(x[0])
                self(x)
                loss = self.recon_loss + self.info_loss + self.kl_loss + l2_loss
                test_loss += loss.data
                test_recon_loss += self.recon_loss.data
                test_kl_loss += self.kl_loss.data         
                test_info_loss += self.info_loss.data               
                
        test_loss /= (i+1)
        test_recon_loss /= (i+1)
        test_kl_loss /= (i+1)
        test_info_loss /= (i+1)
        return test_loss, test_recon_loss, test_kl_loss, test_info_loss
    ...
    def fit(self, train_dataset, valid_dataset,
        ...
        for epoch in range(max_epochs):
            ...
            for i, x in enumerate(train_dl, 0):
                ...
                loss = self.recon_loss + kl_weight * self.kl_loss + (1 - self.a) * kl_weight * self.info_loss + (self.a-1 + self.l) * l2_weight * l2_loss
                ...
                train_info_loss += self.info_loss.data            
            ...
            valid_loss, valid_recon_loss, valid_kl_loss, valid_info_loss = self.test(l2_loss, dl=valid_dl)
            ...
            self.full_loss_store['train_info_loss'][self.epochs]    = float(train_info_loss)
            ...
            self.full_loss_store['valid_info_loss'][self.epochs]    = float(valid_info_loss)
            ...
            if use_tensorboard:
                ...
                writer.add_scalars('1_Loss/3_INFO_Loss' , {'Training' : float(train_info_loss), 
                                                       'Validation' : float(valid_info_loss)}, self.epochs)
